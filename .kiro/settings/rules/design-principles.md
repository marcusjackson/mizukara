...
**Specs Define Contracts, Not Implementation**

Specifications focus on WHAT the system does and WHY, not HOW it's implemented.
...
**Schema Example - Good (Reference)**:

```markdown
<!-- DO: Reference and explain design decisions -->

**Schema**: `/src/db/migrations/001-entries.sql`

**Design Decisions**:

- UUID primary keys enable future device sync without ID conflicts
- Soft deletes preserve temporal history for audit
- Composite index (assigned_day, is_deleted, order_position) optimizes day queries

**Key Fields**:

- `content` (TEXT) - Entry content, unlimited length for flexibility
- `assigned_day` (TEXT) - ISO date for day-based organization
- `order_position` (INTEGER) - Explicit ordering within day

See migration file for complete schema definition.
```
...
Design documents are living documentation and must remain maintainable after implementation completes.

### Implementation Details Removal

Implementation details should NOT be generated in design documents in the first place. Focus on contracts, not code.

**During design review**: Catch and remove algorithms, SQL queries, pseudocode, and step-by-step procedures before implementation begins.

**If details appear**: Extract to `research.md`, reference migration files for SQL, or add references to implementation files.

**Replace with**: Contract descriptions (inputs, outputs, errors, business rules) and references to code (e.g., "See `createEntry()` in entry-mutations.ts").

### Design as Living Documentation

Post-implementation, design documents serve as:

- Source of truth for component contracts and responsibilities
- Reference for maintenance and future changes
- Onboarding documentation for new developers
- Audit trail for architectural decisions

**Spec-first workflow**: During maintenance, update design BEFORE changing code when contracts or architecture changes.
