...
const filteredPeople = computed(() =>
  people.filter((p) => startsWith(p.name, searchTerm.value))
)
</script>
...
<template>
  <ComboboxRoot>
    ...
    <ComboboxPortal>
      <ComboboxContent>
        <ComboboxItem @select.prevent> Item A </ComboboxItem>
        …
      </ComboboxContent>
    </ComboboxPortal>
  </ComboboxRoot>
...
const people = Array.from({ length: 100000 }).map((_, id) => ({
  id,
  name: `Person #${id}`
}))
const selectedPeople = ref(people[0])
...
const filteredPeople = computed(() =>
  people.filter((p) => contains(p.name, searchTerm.value))
)
</script>
